...
export const fetchAllFilteredProducts = createAsyncThunk(
  ...
  async ({ filterParams, sortParams, random = false }) => {
...
export const fetchTopRatedProducts = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(`http://localhost:5001/api/shop/products/top-rated`);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTopRatedProducts.fulfilled, (state, action) => {
        state.productListTopRated  = action.payload.data; 
      })
      .addCase(fetchTopRatedProducts.rejected, (state, action) => {
        state.isLoading = false;
        state.productListTopRated  = [];
      })
  },
});
